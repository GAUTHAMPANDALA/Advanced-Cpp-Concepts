...
void increment(int i) {
 auto now=std::chrono::steady_clock::now();// returns current time.
 if(m.try_lock_until(now + std::chrono::seconds(2))){  // this is same as try_lock for, but this takes current time as reference.
  ++myAmount;
